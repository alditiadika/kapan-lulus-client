      ...
      <Container>
        ...
        <Row>
          ...
          <Col>
            <Card>
              <CardHeader>
                Bidang Studi: <a href="#!">{this.props.auth.user.department}</a>
              </CardHeader>
              <CardBody>
                <p className="mb-3">{this.props.auth.user.university}</p>
                {this.state.row.map(() => (
                  <Row key={Math.random()}>
...
export default connect(mapStateToProps)(FindTeacher);
